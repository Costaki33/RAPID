...
def sec_stream() -> None:
    ...
    for nst in (580, 250):
        w(f"\n### {nst} stations\n")
        w("| Model | NBC CPU | annotate CPU | Model-Actor CPU | MA-NBC CPU | Slipstream-BF16 CPU | "
          "NBC GPU | annotate GPU | Model-Actor GPU | MA-NBC GPU | MA-NBC GPU c2 | 2-GPU Model-Actor |")
        w("|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|")
        for mo in MODELS:
            def warm(method, device, dtype=None, tag=None):
                if tag is not None:
                    wanted_tag = tag
                elif method == "stream_modelactor_2gpu":
...
if __name__ == "__main__":
    ...
    parts = []
    if have_nbc:
        parts.append("Warm Network-Batched Classify cells are included where present.")
    else:
        parts.append("Warm Network-Batched Classify was not measured.")
    if have_ma_nbc:
        parts.append("Model-Actor NBC (stream_modelactor_batched) cells are included where present.")
    nbc_note = " ".join(parts)
