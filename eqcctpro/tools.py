...
# Ray plasma sockets live under {tmpdir}/session_.../sockets/plasma_store (AF_UNIX max 107 bytes).
_RAY_SESSION_SOCKET_TAIL = 72
...
def resolve_ray_temp_dir(
    ...
    *,
    ...
    """Return an absolute temp directory safe for Ray's AF_UNIX socket paths.

    Long workspace paths (e.g. deep clone trees) exceed the 107-byte limit and
  cause ``validate_socket_filename failed``. When the requested path is too long,
    ``/tmp/eqcctpro_ray`` is used instead.
    """
    if tmp_dir is not None and str(tmp_dir).strip():
        candidate = os.path.abspath(os.path.expanduser(str(tmp_dir)))
    else:
        candidate = os.path.abspath(
            os.path.join(os.environ.get("TMPDIR", "/tmp"), "eqcctpro_ray")
        )

    if len(candidate) + _RAY_SESSION_SOCKET_TAIL > 107:
        fallback = "/tmp/eqcctpro_ray"
        if candidate != fallback:
            msg = (
                f"Ray temp dir path too long for AF_UNIX sockets ({len(candidate)} chars): "
                f"{candidate}. Using short fallback: {fallback}"
            )
...
CANONICAL_CSV_HEADER = [
    ...
    "Process Tree RAM (MB)",          # End-of-trial RSS of this PID + all children
    "Peak RAM (MB)",                  # PEAK process-tree RSS during trial (background sampler, PID-isolated)
    "RAM Growth (MB)",                # Peak tree RAM - baseline tree RAM (>=0; RAM this trial's work added)
    "Num Worker Processes",           # Ray workers in this process tree
    # ===== OVERHEAD & EFFICIENCY =====
    # Overhead = Peak actual - Requested (shows Ray infrastructure + framework cost)
    "VRAM Overhead (MB)",             # Process Tree VRAM - Total Requested VRAM
    "RAM Overhead (MB)",              # Peak Process Tree RAM - Total Requested RAM (neg = cap is conservative)
    "VRAM Utilization (%)",           # (Actual / Requested) × 100
    "RAM Utilization (%)",            # (Peak actual / Requested) × 100
    # ===== TIMING METRICS =====
    # Wall-clock phases (whole trial):
    "Total Trial Time (s)",               # Entire trial: setup + actor creation + all timechunk processing
    "Actor Creation Time (s)",            # Wall time to spin up the ModelActor pool (empty for Ripper mode)
    "Total Run time for Picker (s)",      # Wall time of the parallel station-processing loop (excludes actor creation)
    # Per-task breakdown (mean across all station tasks) — shows WHERE in-task time goes:
    "Avg Model Load Time (s)",            # Per-task model load (Ripper loads per task; empty/0 for ModelActor pool)
    "Avg Preprocess Time (s)",            # Per-task waveform read + filter/resample/normalize (mSEED -> model input)
    "Avg Inference Time (s)",             # Per-task model forward pass (predict/classify, incl. actor ray.get round-trip)
    "Avg Pick Write Time (s)",            # Per-task pick post-processing + writing picks to file
    "Avg Task Compute Time (s)",          # Per-task total compute block (preprocess + inference + write + in-task misc)
    # Aggregate compute vs. wall-clock (reveals Ray scheduling / queueing overhead):
    "Sum Task Compute Time (s)",          # Sum of per-task compute across all tasks (serial-equivalent busy time)
    "Task Scheduling Overhead (s)",       # Picker wall time - (Sum Task Compute / effective concurrency); ~Ray queue/routing overhead
    # Measured stage breakdown (busy-second sums across all station tasks, plus
    # the pool warmup wall) — the measured basis for fair per-stage comparisons:
    "Warmup Time (s)",                    # Wall time of the ModelActor pool warmup pass (empty for Ripper: warms per task)
    "Sum Model Load Time (s)",            # Sum of per-task model load (Ripper; 0 for ModelActor pool)
    "Sum Warmup Time (s)",                # Sum of per-task warmup (Ripper; 0 for ModelActor pool)
    "Sum Waveform Load Time (s)",         # Sum of per-task mSEED read / shared-Stream select + 3C assembly
    "Sum Preprocess Time (s)",            # Sum of per-task measured model-input preprocessing (worker or in-actor probes)
    "Sum Inference Time (s)",             # Sum of per-task measured forward (+RPC), minus in-call preprocess/pick segments
    "Sum Pick Write Time (s)",            # Sum of per-task pick extraction + post-processing + file write
    "Successful Inference Tasks",         # Station tasks that completed inference (basis of the per-task sums/means)
    "Total Station Tasks",                # Station tasks attempted
    # ===== PERFORMANCE (Legacy) =====
    "Waveform Processing Time (s)",       # [legacy alias of Avg Preprocess Time] mean per-task waveform load
...
def effective_ram_budget_mb(ram_safety_cap: float = 0.90) -> float:
    """RAM budget (MB) this process is allowed to plan against.

    If the env var ``EQCCTPRO_RAM_BUDGET_MB`` is set to a positive number, it is the
    ABSOLUTE budget (already including whatever headroom you want) and overrides the
    machine-total calculation. This is the mechanism the parallel orchestrator uses to
    hand each concurrently-running strategy a *disjoint slice* of system RAM (e.g.
    ~150 GB each on a 500 GB box) so three sweeps can't collectively oversubscribe and
    OOM the machine. When unset, fall back to ``total_installed_RAM * ram_safety_cap``.
    """
    env = os.environ.get("EQCCTPRO_RAM_BUDGET_MB")
    if env:
        try:
            v = float(env)
            if v > 0:
    ...
    try:
        ...
        return 0.0
    return total_mb * ram_safety_cap


def process_tree_rss_mb(process: psutil.Process = None) -> float:
    """Return the combined RSS (MB) of ``process`` + all its descendants.

    PID-isolated: it only walks THIS process's own child tree, so the value is
    unaffected by other unrelated processes (e.g. sibling strategy sweeps running
    in parallel on different CPU cores). This is the building block for a
    parallel-safe memory metric.
    """
    if process is None:
        process = psutil.Process(os.getpid())
    total = 0
...
def process_tree_pss_mb(process: psutil.Process = None) -> float:
    """Return the combined PSS (MB) of ``process`` + all its descendants.

    PSS (proportional set size) charges each shared page 1/N to each of the N
    processes mapping it, so summing over a tree counts shared memory (Ray
    plasma segments, shared libs, forked pages) exactly once. This makes
    multi-process strategies directly comparable to single-process ones,
    unlike a tree RSS sum which over-counts shared pages once per process.
    Linux-only (reads /proc/<pid>/smaps_rollup); returns 0.0 if unavailable.
    """
    if process is None:
        process = psutil.Process(os.getpid())
    total = 0
...
class ProcessTreeMemorySampler:
    """Background thread that tracks the PEAK PID-isolated process-tree RSS.

    Why this exists
    ---------------
    The old metrics were unreliable when several strategy sweeps run concurrently:
      * ``Peak RAM`` came from ``getrusage(RUSAGE_SELF)`` which only sees the DRIVER
        process, not the Ray workers/actors where almost all memory lives.
      * ``RAM Growth`` was a single after-minus-before snapshot that could land on a
        moment after memory had already been freed, yielding tiny or negative values.

    This sampler instead polls :func:`process_tree_rss_mb` (this PID + all children)
    on a fixed interval for the whole trial and records the high-water mark. Because
    it only ever inspects THIS process's own tree, the number is immune to whatever
    the parallel sibling sweeps are doing. Use it as a context manager::

        with ProcessTreeMemorySampler(process) as sampler:
            ... run the trial ...
        sampler.peak_mb      # true peak process-tree RSS during the trial
        sampler.baseline_mb  # process-tree RSS sampled at sampler start
    """

    def __init__(self, process: psutil.Process = None, interval_s: float = 0.25):
        ...
        self.peak_mb = 0.0
        # PSS twin (shared pages counted once across the tree); see
        # process_tree_pss_mb. Sampled on the same tick as RSS.
        self.baseline_pss_mb = 0.0
    ...
    def _sample(self) -> None:
        cur = process_tree_rss_mb(self.process)
        if cur > self.peak_mb:
            self.peak_mb = cur
        pss = process_tree_pss_mb(self.process)
        self.end_pss_mb = pss
    ...
    def _run(self) -> None:
        while not self._stop.is_set():
            self._sample()
            # Event.wait doubles as the sleep + prompt-stop mechanism.
            self._stop.wait(self.interval_s)

    def start(self) -> "ProcessTreeMemorySampler":
        self.baseline_mb = process_tree_rss_mb(self.process)
        self.peak_mb = self.baseline_mb
        self.baseline_pss_mb = process_tree_pss_mb(self.process)
        self.peak_pss_mb = self.baseline_pss_mb
        self.end_pss_mb = self.baseline_pss_mb
        self._stop.clear()
    ...
    def stop(self) -> float:
        ...
        if self._thread is not None:
            ...
            self._thread = None
        # One final sample so a fast trial still captures its true peak.
        self._sample()
        return self.peak_mb
...
def build_memory_trial_data(
    ...
    peak_tree_ram_mb: float = None,
    baseline_tree_ram_mb: float = None,
) -> Dict[str, any]:
    """
    ...
    # Peak RAM = the TRUE peak of the PID-isolated process tree during the trial.
    # Preferred source: the background ProcessTreeMemorySampler high-water mark
    # (covers Ray workers/actors and is unaffected by parallel sibling sweeps).
    # Fallback: getrusage(RUSAGE_SELF), which only sees the driver process and badly
    # under-reports — used only when no sampler value is supplied.
    if peak_tree_ram_mb is not None and peak_tree_ram_mb > 0:
        peak_ram_value = max(peak_tree_ram_mb, actual_process_ram)
        trial_memory_data["Peak RAM (MB)"] = round(peak_ram_value, 2)
    else:
        peak_ram_value = mem_after.process_peak_mb
        trial_memory_data["Peak RAM (MB)"] = round(peak_ram_value, 2) if peak_ram_value > 0 else ""

    # RAM Growth = peak process-tree RAM minus the baseline sampled before the work
    # began. Both endpoints are PID-isolated, so this is always >= 0 and reflects how
    # much RAM THIS trial's work added on top of the already-running process tree.
    if baseline_tree_ram_mb is not None and peak_tree_ram_mb is not None:
        ram_growth = max(0.0, peak_tree_ram_mb - baseline_tree_ram_mb)
        trial_memory_data["RAM Growth (MB)"] = round(ram_growth, 2)
    elif mem_before is not None:
        # Legacy fallback: single after-minus-before snapshot (can be noisy).
    ...
    # Use the PEAK process-tree RAM (worst-case real usage) vs the theoretical request.
    # A negative overhead is now a meaningful signal that the per-actor RAM estimate
    # (the memory cap) is conservative, not measurement noise.
    ram_for_overhead = max(peak_ram_value or 0.0, actual_process_ram)
    if model_requested_ram_mb is not None and model_requested_ram_mb > 0:
        ram_overhead = ram_for_overhead - model_requested_ram_mb
        trial_memory_data["RAM Overhead (MB)"] = round(ram_overhead, 2)
        
        # RAM Utilization = Actual (peak) / Requested
        if ram_for_overhead > 0:
            ram_efficiency = (ram_for_overhead / model_requested_ram_mb) * 100
...
"""
======================================================================================
JSON Trial Results (canonical orchestration result tracking)
======================================================================================
Orchestration trials are tracked as JSON records in ``trial_results.json`` (one
list entry per trial, in the same directory as the legacy CSV). Field names are
snake_case and orchestration-specific: ``orchestration_strategy``,
``n_modelactors``, ``concurrent_tasks``, ``batch_size``, the per-stage busy
sums, memory, and the trial outcome. The legacy CSV columns map onto these
names via TRIAL_JSON_FIELD_MAP; the CSVs remain only as internal plumbing
(resume keys / optimal-config selection) and are no longer the result record.
"""

TRIAL_RESULTS_JSON_NAME = "trial_results.json"

TRIAL_JSON_FIELD_MAP = {
    # ===== TRIAL CONFIGURATION =====
    "Trial Number": "trial_number",
    "Model Used": "model",
    "Number of Stations Used": "n_stations",
    "Number of CPUs Allocated for Ray to Use": "n_ray_cpus",
    "GPUs Used": "gpus_used",
    "N ModelActors": "n_modelactors",
    "Number of Concurrent Station Tasks": "concurrent_tasks",
    "Actual Ripper Concurrent Tasks": "actual_ripper_concurrent_tasks",
    "Concurrent Timechunks Used": "concurrent_timechunks",
    "Total Number of Timechunks": "n_timechunks",
    "Length of Timechunk (min)": "timechunk_length_min",
    "Total Waveform Analysis Timespace (min)": "analysis_timespan_min",
    # ===== MODEL-REQUESTED MEMORY (theoretical) =====
    "Requested VRAM per Actor (MB)": "requested_vram_per_actor_mb",
    "Requested RAM per Actor (MB)": "requested_ram_per_actor_mb",
    "Total Requested VRAM (MB)": "total_requested_vram_mb",
    "Total Requested RAM (MB)": "total_requested_ram_mb",
    # ===== ACTUAL MEMORY USED (PID-isolated) =====
    "Process Tree VRAM (MB)": "process_tree_vram_mb",
    "Num GPU Processes": "n_gpu_processes",
    "Process Tree RAM (MB)": "process_tree_ram_mb",
    "Peak RAM (MB)": "peak_ram_mb",
    "RAM Growth (MB)": "ram_growth_mb",
    "Num Worker Processes": "n_worker_processes",
    "VRAM Overhead (MB)": "vram_overhead_mb",
    "RAM Overhead (MB)": "ram_overhead_mb",
    "VRAM Utilization (%)": "vram_utilization_pct",
    "RAM Utilization (%)": "ram_utilization_pct",
    # ===== TIMING =====
    "Total Trial Time (s)": "total_trial_time_s",
    "Actor Creation Time (s)": "actor_creation_s",
    "Total Run time for Picker (s)": "picker_wall_s",
    "Avg Model Load Time (s)": "avg_model_load_s",
    "Avg Preprocess Time (s)": "avg_preprocess_s",
    "Avg Inference Time (s)": "avg_inference_s",
    "Avg Pick Write Time (s)": "avg_pick_write_s",
    "Avg Task Compute Time (s)": "avg_task_compute_s",
    "Sum Task Compute Time (s)": "sum_task_compute_s",
    "Task Scheduling Overhead (s)": "task_scheduling_overhead_s",
    "Warmup Time (s)": "warmup_wall_s",
    "Sum Model Load Time (s)": "sum_model_load_s",
    "Sum Warmup Time (s)": "sum_warmup_s",
    "Sum Waveform Load Time (s)": "sum_waveform_load_s",
    "Sum Preprocess Time (s)": "sum_preprocess_s",
    "Sum Inference Time (s)": "sum_inference_s",
    "Sum Pick Write Time (s)": "sum_pick_write_s",
    "Successful Inference Tasks": "successful_inference_tasks",
    "Total Station Tasks": "total_station_tasks",
    "Waveform Processing Time (s)": "avg_waveform_load_s",
    "Intra-parallelism Threads": "intra_threads",
    "Inter-parallelism Threads": "inter_threads",
    "Inference Actor Memory Limit (MB)": "actor_memory_limit_mb",
    # ===== TRIAL OUTCOME =====
    "Trial Success": "trial_success",
...
def trial_results_json_path(csv_path: str) -> str:
    """JSON results file living next to the legacy CSV in the same work dir."""
    return os.path.join(os.path.dirname(os.path.abspath(str(csv_path))), TRIAL_RESULTS_JSON_NAME)


def _trial_json_value(json_key: str, value):
    """Coerce a CSV cell value into a clean JSON value (None / number / list / str)."""
    if value is None or (isinstance(value, str) and value.strip() == ""):
        return None
    if json_key == "gpus_used":
        if isinstance(value, (list, tuple)):
            return [int(g) for g in value]
        try:
            parsed = ast.literal_eval(str(value).replace("(", "[").replace(")", "]"))
    ...
    if isinstance(value, (int, float)):
        # Normalize numpy scalars etc. to plain Python numbers.
        return int(value) if float(value).is_integer() and isinstance(value, int) else float(value)
...
def trial_data_to_json_record(trial_data: dict, extra: dict = None) -> dict:
    """Map a CSV-named trial_data dict onto the snake_case JSON record."""
    record = {}
    for csv_key, value in trial_data.items():
        json_key = TRIAL_JSON_FIELD_MAP.get(csv_key, csv_key)
        record[json_key] = _trial_json_value(json_key, value)
    if extra:
        for k, v in extra.items():
            record[k] = _trial_json_value(k, v)
    return record


def _load_trial_results_json(json_path: str) -> list:
...
def _write_trial_results_json(json_path: str, trials: list) -> None:
...
def append_trial_result_json(csv_path: str, trial_data: dict, extra: dict = None) -> dict:
    """Append one trial record to trial_results.json (same dir as the CSV).

    ``trial_data`` uses the legacy CSV column names (mapped to snake_case);
    ``extra`` carries orchestration-only fields already in snake_case
    (orchestration_strategy, batch_size, use_gpu, ...).
    """
    json_path = trial_results_json_path(csv_path)
    trials = _load_trial_results_json(json_path)
    record = trial_data_to_json_record(trial_data, extra=extra)
    if record.get("trial_number") is None:
        record["trial_number"] = len(trials) + 1
    record["recorded_at"] = datetime.now().isoformat()
    trials.append(record)
    _write_trial_results_json(json_path, trials)
    return record


def update_last_trial_result_json(csv_path: str, patch: dict) -> None:
    """Patch the most recent trial record (success / error / memory fields).

    Accepts legacy CSV column names or snake_case keys; mirrors the
    update_csv / update_csv_with_memory last-row semantics.
    """
    json_path = trial_results_json_path(csv_path)
    trials = _load_trial_results_json(json_path)
    if not trials:
        logging.getLogger(__name__).warning(
            "update_last_trial_result_json: %s has no trial records; skipping patch.", json_path
        )
        return
    trials[-1].update(trial_data_to_json_record(patch))
    _write_trial_results_json(json_path, trials)
...
def _csv_cell_value(value) -> str:
    """Coerce a scalar to a string for CSV columns read with pandas string dtype."""
...
def _trial_success_csv_value(success) -> str:
    return _csv_cell_value(success)
...
def update_csv(csv_filepath, success, error_message):
    ...
    # Ensure string dtype (append_trial_row / QUOTE_NONNUMERIC leaves Trial Success as strings)
    ...
    df.loc[last_idx, 'Trial Success'] = _trial_success_csv_value(success)
    df.loc[last_idx, 'Error Message'] = "" if error_message is None else str(error_message)
...
def update_csv_with_memory(csv_filepath: str, memory_data: Dict[str, any]) -> None:
    ...
    # Update each memory column in the last row (append_trial_row uses string columns)
    for col, value in memory_data.items():
        cell = _csv_cell_value(value)
        if col not in df.columns:
            df[col] = pd.array([""] * len(df), dtype="string")
        elif not pd.api.types.is_string_dtype(df[col]):
            df[col] = df[col].astype("string")
        df.loc[last_idx, col] = cell
...
def find_optimal_configurations_cpu(df):
    ...
    if df_cleaned.empty:
        # No successful trials to optimize over (e.g. every trial failed or the CSV
        # has no data rows). Return empty frames shaped like callers expect instead
        # of raising IndexError, which would abort the entire sweep.
    ...
    # If the moderate-CPU window is empty (e.g. only one CPU count tested), fall
    # back to the full cleaned set so we still pick a best overall configuration.
    if df_moderate_cpus.empty:
        df_moderate_cpus = df_cleaned.copy()
