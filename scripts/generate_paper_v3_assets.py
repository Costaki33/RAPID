#!/usr/bin/env python3
"""Generate figures + tables for the v3 RAPID paper draft from fair-benchmark results.

Reads every result.json under results/fair_benchmark and writes:
  docs/figures_v3/*.png            -- publication figures
  docs/figures_v3/tables_generated.md -- markdown tables to paste into the draft
...
def load_rows():
    ...
    for p in glob.glob(str(RES / "**" / "result.json"), recursive=True):
        ...
        row["conc"] = (reps[0] or {}).get("concurrency")
...
TBL.write("# Generated tables (auto; re-run scripts/generate_paper_v3_assets.py)\n\n")
...
def fig_stage_breakdown():
    ...
    for label, flt in [
        ...
        ("modelactor+\nslip BF16", dict(family="orchestration", method="modelactor_slipstream", dtype="bf16")),
    ]:
...
def fig_orchestration():
    ...
    strategies = [
        ...
        ("modelactor+slip BF16", dict(family="orchestration", method="modelactor_slipstream", dtype="bf16")),
    ]
...
def fig_memory():
    ...
    cats = [("annotate", dict(family="native", method="annotate")),
            ...
            ("MA+slip bf16", dict(family="orchestration", method="modelactor_slipstream", dtype="bf16"))]
    width = 0.16
...
def table_completion():
    ...
    # Final matrix targets (deployment-focused; ripper_slipstream removed,
    # modelactor_slipstream BF16-only, oversub = actor strategies only).
    TARGETS = {
        ("matrix", "annotate"): 1536, ("matrix", "classify"): 384,
        ("matrix", "slipstream"): 6912, ("matrix", "ripper"): 384,
        ("matrix", "modelactor"): 384,
        ("matrix", "modelactor_slipstream"): 1536,
        ("streaming", "stream_modelactor"): 128, ("streaming", "stream_modelactor_slipstream"): 256,
        ("oversub", "modelactor"): 192, ("oversub", "modelactor_slipstream"): 192,
    }
    ...
    order = [("matrix", "annotate"), ("matrix", "classify"), ("matrix", "slipstream"),
             ...
             ("matrix", "modelactor_slipstream"),
             ("streaming", "stream_modelactor"), ("streaming", "stream_modelactor_slipstream"),
             ("oversub", "modelactor"), ("oversub", "modelactor_slipstream")]
    tot_done = tot_tgt = 0
...
def table_gpu_orchestration():
    ...
    TBL.write("| Model | Ripper | Model-Actor | MA+Slip BF16 |\n")
    TBL.write("|---|---:|---:|---:|\n")
    for model in MODELS:
        reg = canonical_regime(model)
        cells = []
        for flt in (dict(family="orchestration", method="ripper"),
                    ...
                    dict(family="orchestration", method="modelactor_slipstream", dtype="bf16")):
    ...
    TBL.write("\nGPU cells: best over host-core budgets. Final GPU-isolation-corrected data.\n\n")
...
def table_orchestration():
    ...
    TBL.write("| Model | classify (native) | Ripper | Model-Actor | MA+Slip BF16 |\n")
    TBL.write("|---|---:|---:|---:|---:|\n")
    for model in MODELS:
        reg = canonical_regime(model)
        cells = []
...
def fig_inference_speedup():
    """Slipstream's REAL win: inference-stage speedup over annotate (not cold-start
    total, which fixed model-load dominates). Mean over models, CPU and GPU."""
    ...
    for i, dev in enumerate(("cpu", "gpu")):
        ...
        for model in MODELS:
            reg = canonical_regime(model)
            kw = dict(bucket="matrix", model=model, ds="stead", st=580, dev=dev)
            ann = [r for r in sel(ROWS, method="annotate", **kw)
                   if r["regime"] == reg and (dev == "gpu" or r["ncpu"] == 20)]
            sb = [r for r in sel(ROWS, method="slipstream", dtype="bf16", **kw)
                  if r["regime"] == reg and not r["comp"] and (dev == "gpu" or r["ncpu"] == 20)]
            a = min((r["inference"] for r in ann if r["inference"]), default=0)
            s = min((r["inference"] for r in sb if r["inference"]), default=0)
            vals.append(a / s if s > 0 else 0)
        ax.bar([xi + (i - 0.5) * width for xi in x], vals, width, label=f"{dev.upper()}")
    ax.axhline(1.0, color="k", lw=0.8, ls=":")
    ax.set_xticks(list(x)); ax.set_xticklabels(MODELS, fontsize=9)
    ax.set_ylabel("inference-stage speedup\n(annotate / Slipstream BF16)")
    ax.set_title("Slipstream BF16 inference speedup over annotate() — STEAD 580 st\n"
                 "(the forward-path win; cold-start total is masked by fixed model load)")
    ax.legend()
    fig.tight_layout()
    fig.savefig(OUT / "fig_inference_speedup.png", bbox_inches="tight")
    plt.close(fig)


def fig_oversub():
    """Oversubscription: requested concurrency vs achieved actor pool and runtime.
    CPU (RAM-bound) and GPU (VRAM-bound), PhaseNet modelactor_slipstream."""
    fig, axes = plt.subplots(1, 2, figsize=(11, 4))
    for ax, dev in zip(axes, ("cpu", "gpu")):
        rows = [r for r in ROWS if r["bucket"] == "oversub"
                and r["method"] == "modelactor_slipstream" and r["model"] == "PhaseNet"
                and r["dev"] == dev and r.get("conc") and r.get("n_modelactors")]
        # group by host-core budget
        budgets = sorted({r["ncpu"] for r in rows})
        ax2 = ax.twinx()
        for c in budgets:
            pts = sorted([r for r in rows if r["ncpu"] == c], key=lambda r: r["conc"])
            if not pts:
                continue
            xs = [r["conc"] for r in pts]
            ax.plot(xs, [r["n_modelactors"] for r in pts], "o-", ms=4, label=f"{c} cores")
            ax2.plot(xs, [r["total"] for r in pts], "s--", ms=3, alpha=0.5)
        ax.plot([0, max((r["conc"] for r in rows), default=1)],
                [0, max((r["conc"] for r in rows), default=1)], "k:", lw=0.7, alpha=0.5)
        ax.set_xlabel("requested concurrency (multiplier x cores)")
        ax.set_ylabel("achieved actors (solid)")
        ax2.set_ylabel("total wall time s (dashed)")
        ax.set_title(f"{dev.upper()} — {'RAM' if dev=='cpu' else 'VRAM'}-bound")
        ax.legend(fontsize=7, title="core budget")
    fig.suptitle("Oversubscription: achieved actor pool & runtime vs requested concurrency\n"
                 "(PhaseNet modelactor_slipstream, STEAD 580 st; diagonal = request met)", fontsize=10)
    fig.tight_layout()
    fig.savefig(OUT / "fig_oversub.png", bbox_inches="tight")
    plt.close(fig)
