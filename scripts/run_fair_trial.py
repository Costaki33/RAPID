...
* ``classify`` -- SeisBench's built-in ``model.classify()`` END-TO-END, one
  ...
  scheme sets the torch thread budget only (no worker pool, no batching).
...
NATIVE_METHODS = ("annotate", "classify", "slipstream")
...
def run_driver(args) -> int:
    ...
    meta = dict(
        ...
        pick_extractor=("seisbench_classify" if args.method == "classify" else "rapid_threshold_crossing"),
