...
cell() {  # strategy dtype device model cores mult
  local strat=$1 dt=$2 dev=$3 model=$4 c=$5 mult=$6
  ...
  note "OVERSUB $strat/$dt $dev $model cpu$c x${mult}->${cc}act"
  python3 scripts/run_fair_orch_trial.py \
    ...
    --repeats 2 --tag "iso_ov_${dev}_cpu${c}_c${mt}x" \
    --results-root "$ROOT" --resume --dedup-vram-capped >> "$LOG" 2>&1
}

note "ISOLATION OVERSUB START"
# modelactor (fp32) first -- the headline CPU result -- then slipstream (bf16).
