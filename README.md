...
Improved warm latency from persistent workers comes with a corresponding increase in host memory. Peak process-tree proportional set size (PSS) was used so that pages shared among Ray workers are counted once, allowing single-process and orchestrated runs to be compared on the same footing. The values Improving warm-streaming latency through persistent Model-Actor workers comes at the cost of additional host memory. To fairly compare memory usage between the single-process and orchestration strategies, we measured the peak process-tree proportional set size (PSS), which counts memory pages shared among Ray workers only once. This allows shared memory to be accounted for consistently across all methods. The values reported below represent the mean peak PSS in gigabytes for the 580-station STEAD workload using a 20-core CPU allocation. Cold-start measurements capture the maximum memory usage while the model pool is being initialized, whereas warm-streaming measurements represent continuous operation after the one-time initialization cost has already been paid. are mean peak PSS in GB for the 580-station STEAD workload on a twenty-core CPU allocation. Cold-start peaks capture the highest footprint while models are being loaded into the pool; warm peaks reflect a kept-alive streaming run after that initialization cost has been paid.
...
Single-process inference remained near 1 GB because only a single model instance was loaded into memory. In contrast, Model-Actor maintains one model replica per persistent worker, resulting in a substantially larger memory footprint during cold start while the worker pool is being populated. Once initialization was complete, the warm-streaming footprint stabilized between approximately 11 and 12 GB for the 20-worker pool. Using BF16 Slipstream did not noticeably reduce CPU host-memory usage compared to Classify because the replicated model parameters dominate the overall footprint. Instead, BF16 primarily reduces GPU memory usage. Although Ripper avoids maintaining a persistent worker pool and therefore requires less memory than Model-Actor during cold start, this comes at the expense of substantially higher runtime, making it a poor tradeoff for continuously running monitoring systems. Because memory usage scales with the number of persistent workers, these results further support sizing the actor pool to approximately one worker per available CPU core.

Taken together, the runtime and memory results indicate that the preferred deployment strategy depends on both the available hardware and the host-memory budget. On CPU-only systems, persistent Model-Actor workers provide the most practical approach for achieving real-time processing of large station networks, provided sufficient memory is available to maintain the replicated model pool. When a GPU is available, however, single-process Annotate remained the fastest configuration evaluated while also requiring substantially less host memory than Model-Actor.
